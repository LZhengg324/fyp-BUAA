...
class ContourCard:
    def __init__(self, server: Server, source_manager: SourceManager,
        ...
        self.state.Contour_contour_by = ""
        self.state.Contour_lower = 0
        self.state.Contour_upper = 0
        ...
        @self.state.change('Contour_contour_by')
        def update_selected_item(Contour_contour_by, **kwargs):
            print(Contour_contour_by)
            if self.state.point_data_range[Contour_contour_by]:
                self.state.Contour_lower = self.state.point_data_range[Contour_contour_by][0]
                self.state.Contour_upper = self.state.point_data_range[Contour_contour_by][1]
    ...
    def show_card(self):
        ...
        with UICard(card_type=CardType.Contour, data_holder=self.data_holder).ui_card():
            with vuetify.VRow():
                with vuetify.VCol(
                    cols=4
                ):
                ...
                with vuetify.VCol(
                  cols=10
                    ...
                    vuetify.VList(
                        ...
                        items=("Isosurfaces[Contour_contour_by]", {}),
                        dense=True,
            ...
            vuetify.VLabel(
                "Range:[{{Contour_lower}} ,{{Contour_upper}}] ",
    ...
    def add_contour_value(self):
        ...
        t[self.state.Contour_contour_by] = t[self.state.Contour_contour_by] + [
            {"title": str(new_value), "value": new_value}, ]
        self.state.Isosurfaces = t

    def minus_contour_value(self):
        print("used minus_contour_value")
        if len(self.state.Isosurfaces[self.state.Contour_contour_by]) == 1:
            return
        t, target = self.get_deepcopy_and_target()
        for i in range(len(t[self.state.contour_by]) - 1, -1, -1):
            if t[self.state.Contour_contour_by][i]["value"] == target:
                del t[self.state.Contour_contour_by][i]
                break
        self.state.Isosurfaces = t
    ...
    def get_deepcopy_and_target(self):
        ...
        if type(self.state.selected_item) is list and self.state.selected_item:
            ...
            target = t[self.state.Contour_contour_by][-1]["value"]
        return t, target
    ...
    def modify_contour_props(self):
        isosurfaces = list(float(self.state.Isosurfaces[self.state.Contour_contour_by][i]["value"]) for i in
                           range(len(self.state.Isosurfaces[self.state.Contour_contour_by])))
        self.source_manager.modify_contour_props(source_id=self.state.active_view, cur_mesh=self.state.cur_mesh,
                                                 contour_by=self.state.Contour_contour_by, Isosurfaces=isosurfaces)
        self.data_holder.write_in(self.state.active_view)
        self.visible_manager.update_view()

    def generate_batch_isosurfaces(self):
        t = deepcopy(self.state.Isosurfaces)
        ran = np.random.uniform(low=self.state.Contour_lower,
                        high=self.state.Contour_upper,
                        size=int(self.state.linear_num)).tolist()
        l = []
        for r in ran:
            l.append({"title": str(r), "value": r})
        t[self.state.Contour_contour_by] = t[self.state.Contour_contour_by] + l
        self.state.Isosurfaces = t
        self.state.batch_dialog = False
        self.state.linear_num = 10